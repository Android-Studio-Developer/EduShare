...
class PhaseVocoderProcessor extends OLAProcessor {
    ...
    constructor(options) {
        ...
        // Formant-shift buffers: independent spectral-envelope (formant) control,
        // layered in front of the peak-based pitch shift above. Without this, a
        // single pitchFactor knob moves pitch AND formants together, which is
        // physiologically wrong for a male->female voice (real female voices have
        // higher pitch but only slightly higher formants) and reads as "chipmunk"
        // rather than "girl" once you push it far enough to matter.
        // ~1ms cutoff quefrency — keeps the envelope smooth enough to trace
        // formants (spaced hundreds of Hz apart) without also tracing individual
        // pitch harmonics (spaced ~1 F0 apart).
        this.formantOrder = Math.round(sampleRate * 0.001);
        this.cepstrumInput = this.fft.createComplexArray();
        this.cepstrumBuffer = this.fft.createComplexArray();
        this.envelopeComplex = this.fft.createComplexArray();
        this.envelope = new Float32Array(this.fftSize / 2 + 1);
    }

    processOLA(inputs, outputs, parameters) {
        // no automation, take last value
        const pitchFactor = parameters.pitchFactor[parameters.pitchFactor.length - 1];
        const formantFactor = parameters.formantFactor[parameters.formantFactor.length - 1];
        ...
        for (var i = 0; i < this.nbInputs; i++) {
            for (var j = 0; j < inputs[i].length; j++) {
                ...
                if (formantFactor !== 1) {
                    this.shiftFormants(formantFactor);
                    this.computeMagnitudes();
                }
                this.findPeaks();
    ...
    /** Estimate a smooth spectral envelope (the formant shape) via cepstral
     * liftering: log-magnitude -> IFFT (cepstrum) -> zero out everything but
     * the low-quefrency coefficients -> FFT back -> exponentiate. */
    computeEnvelope() {
        const halfSize = this.magnitudes.length;

        this.cepstrumInput.fill(0);
        for (var k = 0; k < halfSize; k++) {
            this.cepstrumInput[k * 2] = 0.5 * Math.log(this.magnitudes[k] + 1e-9);
        }
        this.fft.completeSpectrum(this.cepstrumInput);
        this.fft.inverseTransform(this.cepstrumBuffer, this.cepstrumInput);

        const order = this.formantOrder;
        for (var q = order; q < this.fftSize - order; q++) {
            this.cepstrumBuffer[q * 2] = 0;
            this.cepstrumBuffer[q * 2 + 1] = 0;
        }

        this.fft.transform(this.envelopeComplex, this.cepstrumBuffer);
        for (var k2 = 0; k2 < halfSize; k2++) {
            this.envelope[k2] = Math.max(Math.exp(this.envelopeComplex[k2 * 2]), 1e-6);
        }
    }

    /** Move the spectral envelope (formants) independently of pitch by
     * resampling it in frequency and re-applying it to the spectrum, keeping
     * the underlying harmonic content (divided out as the "residual") intact. */
    shiftFormants(formantFactor) {
        this.computeEnvelope();
        const halfSize = this.magnitudes.length;

        for (var k = 0; k < halfSize; k++) {
            const srcPos = k / formantFactor;
            let shiftedEnvelope;
            if (srcPos >= halfSize - 1) {
                shiftedEnvelope = this.envelope[halfSize - 1];
            } else if (srcPos < 0) {
                shiftedEnvelope = this.envelope[0];
            } else {
                const i0 = Math.floor(srcPos);
                const frac = srcPos - i0;
                shiftedEnvelope = this.envelope[i0] * (1 - frac) + this.envelope[i0 + 1] * frac;
            }

            let gain = shiftedEnvelope / this.envelope[k];
            if (!isFinite(gain)) gain = 1;
            gain = Math.min(Math.max(gain, 0), 8);

            this.freqComplexBuffer[k * 2] *= gain;
            this.freqComplexBuffer[k * 2 + 1] *= gain;
        }
    }
