...
import { COSMETIC_CATALOG, createRedeemCode, subscribeToRedeemCodes } from "../lib/redeemCodes";
import { ALL_COSMETICS, CAPES } from "../lib/cosmetics";
...
function BotEditor({ bot, onSaved, onDeleted }: { bot: DeveloperBot; onSaved: (bot: DeveloperBot) => void; onDeleted: () => void }) {
  ...
  const [editorMode, setEditorMode] = useState<"easy" | "form">("form");
  ...
  function installBuilderTemplate(source: string) {
    const result = compileEduScript(source);
    if (result.errors.length) { setNotice(result.errors[0]); return; }
    setEditorMode("form");
    setEasyCode(source);
    set("commands", result.commands);
    setNotice("No-code template loaded. Click Save changes to publish it.");
  }
  ...
  async function save() {
    ...
    if (!prepared.commands.some((command) => command.name.trim() && (command.response.trim() || command.panel || command.conditions?.some((condition) => condition.response.trim())))) return setNotice("Add at least one complete command or panel.");
  ...
  return <section className="overflow-hidden rounded-2xl border border-white/10 bg-[#11151d]/90">
    ...
    <div className="grid gap-5 p-5 lg:grid-cols-[1fr_.85fr]">
      ...
      <div>
        ...
        {editorMode === "easy" ? <div><div className="rounded-xl border border-emerald-400/15 bg-emerald-400/[.04] p-3"><div className="flex items-center justify-between gap-3"><p className="text-xs font-bold text-emerald-200">EduPy Studio — safe app commands</p><Link to="/develop/learn" className="text-[11px] font-bold text-indigo-300 hover:text-indigo-200">Open interactive tutor →</Link></div><p className="mt-1 text-[11px] leading-5 text-white/40">Use <code className="text-violet-200">command join:</code>, <code className="text-cyan-200">say</code>, <code className="text-cyan-200">panel</code>, <code className="text-cyan-200">field</code>, <code className="text-cyan-200">button</code>, and <code className="text-emerald-200">authenticate user</code>. It compiles to safe chat commands — no dangerous tokens or arbitrary code.</p><div className="mt-3 grid gap-2">{eduScriptTemplates.map((template) => <button key={template.id} type="button" onClick={() => installTemplate(template.source)} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-left hover:border-brand-300/35 hover:bg-brand-500/10"><span className="block text-xs font-bold text-white">{template.name}</span><span className="mt-0.5 block text-[10px] leading-4 text-white/35">{template.description}</span></button>)}</div></div><textarea value={easyCode} onChange={(event) => setEasyCode(event.target.value)} spellCheck={false} aria-label="EduPy code" className="mt-3 min-h-[360px] w-full resize-y rounded-xl border border-white/10 bg-[#080b10] p-4 font-mono text-xs leading-6 text-white/80 outline-none focus:border-brand-400/55" placeholder={eduScriptTemplates[0].source}/><div className={`mt-2 rounded-lg px-3 py-2 text-[11px] ${compiledPreview.errors.length ? "bg-red-500/10 text-red-200" : "bg-emerald-500/10 text-emerald-200"}`}>{compiledPreview.errors[0] ?? `${compiledPreview.commands.length} command${compiledPreview.commands.length === 1 ? "" : "s"} ready${compiledPreview.commands.some((command) => command.action === "verify") ? " · authentication enabled" : ""}.`}</div>{compiledPreview.commands.length > 0 && <div className="mt-3 grid gap-2">{compiledPreview.commands.map((command) => <div key={command.name} className="rounded-xl border border-white/8 bg-black/20 p-3"><div className="flex items-center justify-between gap-2"><span className="font-mono text-xs font-bold text-white">/{draft.handle || "bot"} {command.name}</span><span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase ${command.action === "verify" ? "bg-emerald-400/10 text-emerald-200" : "bg-white/8 text-white/45"}`}>{command.action === "verify" ? "auth" : "reply"}</span></div>{command.panel ? <BotMessagePanel panel={command.panel}/> : <p className="mt-2 whitespace-pre-wrap text-xs leading-5 text-white/50">{command.response}</p>}</div>)}</div>}</div> : <><div className="mb-3 rounded-xl border border-brand-400/15 bg-brand-500/[.04] p-3"><p className="flex items-center gap-2 text-xs font-bold text-brand-100"><Rocket size={13}/>No-code app templates</p><p className="mt-1 text-[11px] leading-5 text-white/40">Pick one, change the text boxes, then save. Your friends do not need to touch EduPy.</p><div className="mt-3 grid gap-2 sm:grid-cols-3">{eduScriptTemplates.map((template) => <button key={template.id} type="button" onClick={() => installBuilderTemplate(template.source)} className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-left hover:border-brand-300/40 hover:bg-brand-500/10"><span className="block text-[11px] font-black text-white">{template.name}</span><span className="mt-1 block text-[10px] leading-4 text-white/35">{template.description}</span></button>)}</div></div><div className="mb-2 flex justify-end"><button type="button" disabled={draft.commands.length >= 12} onClick={() => set("commands", [...draft.commands, { name: "", response: "", action: "reply" }])} className="cursor-target flex items-center gap-1 rounded-lg px-2 py-1 text-xs text-brand-300 hover:bg-brand-500/10 disabled:opacity-30"><Plus size={12}/>Add command</button></div><div className="max-h-[390px] space-y-2 overflow-y-auto pr-1">{draft.commands.map((command, index) => <div key={index} className="rounded-xl border border-white/8 bg-black/20 p-3"><div className="flex gap-2"><input value={command.name} onChange={(event) => setCommand(index, "name", event.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, ""))} placeholder="command" maxLength={24} className="min-w-0 flex-1 rounded-lg border border-white/8 bg-black/20 px-2.5 py-2 font-mono text-xs text-white outline-none focus:border-brand-400/50"/><select value={command.action ?? "reply"} onChange={(event) => { const value = event.target.value; const action = value === "verify" || value === "redeem" || value === "buy" ? value : "reply"; set("commands", draft.commands.map((item, itemIndex) => itemIndex === index ? { ...item, action } : item)); }} className="rounded-lg border border-white/8 bg-[#11151d] px-2 text-[11px] text-white"><option value="reply">Reply</option><option value="verify">Verify/Auth</option><option value="redeem">Redeem code</option><option value="buy">Buy cosmetic</option></select><button type="button" aria-label="Remove command" onClick={() => set("commands", draft.commands.filter((_, itemIndex) => itemIndex !== index))} className="rounded-lg p-2 text-white/25 hover:bg-red-500/10 hover:text-red-300"><X size={14}/></button></div>{command.action === "buy" ? <select value={command.cosmeticId ?? ""} onChange={(event) => setCommand(index, "cosmeticId", event.target.value)} className="mt-2 w-full rounded-lg border border-white/8 bg-black/20 px-2.5 py-2 text-xs text-white outline-none focus:border-brand-400/50"><option value="">Pick a cosmetic to sell…</option>{CAPES.length > 0 && <optgroup label="Turbo Client capes">{CAPES.map((item) => <option key={item.id} value={item.id}>{item.label} — {item.cost}cr</option>)}</optgroup>}<optgroup label="Everything else">{ALL_COSMETICS.filter((item) => item.category !== "cape").map((item) => <option key={item.id} value={item.id}>{item.label} — {item.cost}cr</option>)}</optgroup></select> : <textarea value={command.response} onChange={(event) => setCommand(index, "response", event.target.value)} placeholder="Response text. Use {user} for their name and {args} for what they typed." maxLength={300} rows={3} className="mt-2 w-full resize-none rounded-lg border border-white/8 bg-black/20 px-2.5 py-2 text-xs text-white outline-none focus:border-brand-400/50"/>}</div>)}</div></>}
      </div>
    </div>
...
function RedeemCodesPanel({ userId }: { userId: string }) {
  const [codes, setCodes] = useState<RedeemCode[]>([]);
  const [cosmeticId, setCosmeticId] = useState(COSMETIC_CATALOG[0].id);
  const [busy, setBusy] = useState(false);
  const [lastCode, setLastCode] = useState("");
  const [notice, setNotice] = useState("");

  useEffect(() => subscribeToRedeemCodes(setCodes), []);
  ...
  async function generate() {
    ...
    setNotice("");
    try {
      const code = await createRedeemCode(cosmetic.id, cosmetic.label, userId);
      setLastCode(code);
    } catch (error) {
      setNotice(error instanceof Error ? error.message : "Could not generate a code.");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="mt-6 rounded-2xl border border-white/10 bg-[#11151d]/90 p-5">
      <div className="flex items-center gap-2"><Gift size={16} className="text-amber-300"/><h2 className="font-bold text-white">Cosmetic redeem codes</h2></div>
      <p className="mt-1 text-xs leading-5 text-white/40">Generate a one-time code for a cosmetic cape. Give it to a player, add a bot command with the <b className="text-white/70">Redeem code</b> action so they can type it in chat.</p>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <select value={cosmeticId} onChange={(event) => setCosmeticId(event.target.value)} className="rounded-lg border border-white/10 bg-black/25 px-3 py-2 text-sm text-white outline-none">
          {COSMETIC_CATALOG.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
        </select>
        <Button size="sm" onClick={() => void generate()} disabled={busy}><Gift size={13}/>{busy ? "Generating…" : "Generate code"}</Button>
        {lastCode && <button type="button" onClick={() => void navigator.clipboard?.writeText(lastCode)} className="cursor-target flex items-center gap-1.5 rounded-lg border border-emerald-300/25 bg-emerald-400/10 px-3 py-2 font-mono text-sm text-emerald-200"><Copy size={13}/>{lastCode}</button>}
      </div>
      {notice && <p className="mt-2 text-xs text-red-300">{notice}</p>}
      <div className="mt-4 max-h-56 space-y-1.5 overflow-y-auto pr-1">
        {codes.slice(0, 30).map((item) => (
          <div key={item.id} className="flex items-center justify-between gap-3 rounded-lg bg-black/15 px-3 py-1.5 text-xs">
            <span className="font-mono text-white/70">{item.code}</span>
            <span className="text-white/40">{item.cosmeticLabel}</span>
            <span className={item.used ? "text-white/30" : "text-emerald-300/80"}>{item.used ? "Redeemed" : "Unused"}</span>
          </div>
        ))}
        {codes.length === 0 && <p className="py-4 text-center text-xs text-white/25">No codes generated yet.</p>}
      </div>
    </section>
  );
}
...
export default function Develop() {
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:py-12">
    <section className="relative overflow-hidden rounded-3xl border border-brand-400/20 bg-[#0b1020]/90 px-6 py-10 sm:px-10">
      ...
      <div className="relative max-w-3xl"><span className="inline-flex items-center gap-2 rounded-full border border-brand-300/20 bg-brand-500/10 px-3 py-1 font-mono text-[11px] uppercase tracking-[.18em] text-brand-200"><Sparkles size={12}/>SpawnDex Developer Platform</span><h1 className="mt-5 text-3xl font-black tracking-tight text-white sm:text-5xl">Build real chat apps, not boring replies.</h1><p className="mt-4 max-w-2xl text-sm leading-7 text-white/55 sm:text-base">Create Discord-style app bots with join/authentication panels, command templates, safe EduPy logic, health checks, and live telemetry—without exposing tokens or member data.</p><div className="mt-6 flex flex-wrap items-center gap-3 text-xs text-white/45"><Link to="/develop/learn" className="inline-flex items-center gap-2 rounded-lg bg-[#5865f2] px-4 py-2.5 font-bold text-white hover:bg-[#4752c4]"><Code2 size={14}/>Learn EduPy step by step</Link><span className="flex items-center gap-1.5"><ShieldCheck size={14} className="text-emerald-300"/>Staff-reviewed developers</span><span className="flex items-center gap-1.5"><Rocket size={14} className="text-cyan-300"/>Join/auth panels</span>{ownerDebugAccess && <span className="flex items-center gap-1.5 rounded-full border border-amber-300/20 bg-amber-400/10 px-2 py-0.5 font-bold text-amber-200"><KeyRound size={13}/>Owner debugging mode</span>}</div></div>
    </section>

    {user && <AccountSecurityPanel user={user} bots={bots}/>} 

    {isStaff && user && <RedeemCodesPanel userId={user.uid}/>}
