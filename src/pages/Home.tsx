...
export default function Home() {
  ...
  useEffect(() => {
    const unsub = subscribeToServers((nextServers) => {
  ...
  const filtered = useMemo(() => {
    return servers.filter((server) => {
      const matchesSubject = subject === "All" || server.subject === subject;
      const matchesSearch =
  ...
  const partners = useMemo(
    () => servers.filter((server) => server.isPartner),
    [servers],
  );
  ...
  return (
    <div>
      ...
      <section className="relative mx-auto max-w-6xl px-6 pt-20 pb-16 text-center sm:pt-28">
        ...
        >
          Share your{" "}
          <span className="font-editorial text-shimmer italic">Minecraft Edu</span>{" "}
          world.
        ...
        <motion.p
          ...
          className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-indigo-100/55 sm:text-lg"
        >
          Discover colorful classroom worlds, meet creative builders, and jump
          into your next Minecraft Education adventure.
        </motion.p>
      ...
      <section id="browse" className="mx-auto max-w-6xl px-6 pb-24">
        <div className="glass-panel mb-8 rounded-2xl">
          <ActivityFeed />
        </div>
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="font-mono text-xs font-bold tracking-[.1em] text-violet-300/80 uppercase">
                        eduShare Partner
                      </p>
                ...
                {partner.partnerPerks && (
                  <p className="mt-3 text-sm text-white/70">{partner.partnerPerks}</p>
                )}
              </Link>
            ))}
          </div>
        ...
        <div className="mb-8 rounded-2xl border border-border bg-surface p-6">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <p className="flex items-center gap-2 font-mono text-sm font-bold text-white">
              <Sparkles size={15} className="text-brand-400" /> What's new —{" "}
              {LATEST_PATCH_NOTE.version}
            </p>
            <Link
              to="/changelog"
              className="cursor-target text-xs text-brand-400 hover:underline"
            >
              Full changelog →
            </Link>
          </div>
        ...
        <div className="mb-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            {(["All", ...SUBJECTS] as const).map((nextSubject) => (
              <button
                key={nextSubject}
                onClick={() => setSubject(nextSubject)}
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-border py-20 text-center text-white/40">
            ...
            <Link
              to="/create"
              className="cursor-target text-brand-400 hover:underline"
            >
              register one
            </Link>
            .
